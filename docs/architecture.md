...
Plain CommonJS, no build step. One window is the app: the companion (the orb,
with the chat panel attached to it). Its header opens a small models pane —
which model, voice and ear — and that is the only settings surface; routines,
automations, browser exclusions, the colour and what fren remembers are all
managed by asking in the chat (`main/own-business.js`). The main process owns all state and all capture; the renderer
